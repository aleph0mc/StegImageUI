...
namespace StegImageUI.Helpers
{
    public class TifHelper
    {
        ...
        public static void StegBinary(string TifFilePath, BitArray Bits, int SecurityLevel)
        {
            var bitmap = new Bitmap(TifFilePath);
            ...
            var flPath = System.IO.Path.GetDirectoryName(TifFilePath);
            var filename = TifFilePath.Remove(0, string.Concat(flPath, "\\").Length);
            var docPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            var newFileNameAndPath = $@"{docPath}\StegImageUI\steg_{DateTime.Now.ToString("yyyyMMddHHmmss")}_{filename}";
            bitmap.Save(newFileNameAndPath, ImageFormat.Tiff);
        ...
        public static StegStruct UnstegBinary(string TifFilePath)
        {
